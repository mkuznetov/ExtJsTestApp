(function() {
    ...
    var Controller = {
        startTestRun: function (message) {
            ...
            if (ST.LOGME) console.log('options.driverConfig='+JSON.stringify(options.driverConfig));

            // if (options.xdriverConfig) { // skip for now to let me start my own context to test ie startup on win10
        ...
        stopRecording: function() {
            if (ST.defaultContext) {
                ST.defaultContext.stopRecording().then(function (ret) {
                    ST.sendMessage({type: 'recordingStopped'});
                });
            } else if (ST.recorder) {
                ST.warnOnLeave(false);
                ST.recorder.stop();
