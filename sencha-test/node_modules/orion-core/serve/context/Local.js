ST.context.Local = ST.define({
    ...
    expandTap: function (event) {
        ...
        if (cmp && cmp.isXType && cmp.isXType('menuitem')) {
            visibilityMode = null;
        }
        ...
        // allow special "visibilityMode" for things like menuitems that will no longer be visible before the full sequence has finished
        // when visibilityMode is null, we'll not enforce visibility on pointerup/click events
        tapEvents = [
            { type: 'pointerdown', target: target, delay: event.delay, x: x, y: y },
            { type: 'pointerup',   target: -1, delay: 0, x: x, y: y, visible: visibilityMode },
            { type: 'click',       target: -2, delay: 0, x: x, y: y, visible: visibilityMode },
